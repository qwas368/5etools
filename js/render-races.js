class RenderRaces {
	static $getRenderedRace (race) {
		...
		${!race._isBaseRace ? `<tr><td colspan="6"><b>屬性值:</b> ${(race.ability ? Renderer.getAbilityData(race.ability) : {asText: "無"}).asText}</td></tr>
		...
		${race.traitTags && race.traitTags.includes("NPC Race") ? `<tr class="text"><td colspan="6"><section class="text-muted">
			${renderer.render(`{@i 註記： 這個種族被記於《地下城主指南》以做為創造非玩家角色的選項。它並非被設計做為玩家可用的種族。}`, 2)}
		 </section></td></tr>` : ""}
