class RenderSpells {
	static $getRenderedSpell (sp, subclassLookup) {
		...
		if (fromClassList.length) {
			...
			renderStack.push(`<tr class="text"><td colspan="6"><span class="bold">職業: </span>${Parser.spMainClassesToFull(current)}</td></tr>`);
			if (legacy.length) renderStack.push(`<tr class="text"><td colspan="6"><section class="text-muted"><span class="bold">職業 (舊版): </span>${Parser.spMainClassesToFull(legacy)}</section></td></tr>`);
		}
		...
		if (fromSubclass.length) {
			...
			renderStack.push(`<tr class="text"><td colspan="6"><span class="bold">子職業: </span>${current}</td></tr>`);
			if (legacy.length) {
				renderStack.push(`<tr class="text"><td colspan="6"><section class="text-muted"><span class="bold">子職業 (舊版): </span>${legacy}</section></td></tr>`);
			}
		}

		const fromClassListVariant = Renderer.spell.getCombinedClasses(sp, "fromClassListVariant");
		if (fromClassListVariant.length) {
			renderStack.push(`<tr class="text"><td colspan="6"><span class="bold" title="Source: ${Parser.sourceJsonToFull(SRC_UACFV)}">變體職業: </span>${Parser.spMainClassesToFull(fromClassListVariant)}</td></tr>`);
		}
		...
		if (sp.backgrounds) {
			...
			renderStack.push(`<tr class="text"><td colspan="6"><span class="bold">背景: </span>${sp.backgrounds.map(r => `${SourceUtil.isNonstandardSource(r.source) ? `<span class="text-muted">` : ``}${renderer.render(`{@background ${r.name}|${r.source}|${Parser.translate(PageFilterSpells.SP_BACKGROUND_TO_TRANS, r.name)}}`)}${SourceUtil.isNonstandardSource(r.source) ? `</span>` : ``}`).join(", ")}</td></tr>`);
		}
